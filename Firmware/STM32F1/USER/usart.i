...
{      
	while((((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400))->SR&0X40)==0){}
    ((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400))->DR = (u8) ch;      
	return ch;
}
...
void uart1_init(u32 bound){
	...
	RCC_APB2PeriphClockCmd(((uint32_t)0x00004000)|((uint32_t)0x00000004), ENABLE);	
...
void USART1_IRQHandler(void){
	...
	if(USART_GetITStatus(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)), ((uint16_t)0x0525)) != RESET){
		Res = USART_ReceiveData(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3800)));
		if((USART1_RX_STA&0x8000)==0){
			
			if(USART1_RX_STA&0x4000){
				if((USART1_RX_STA&0X3FFF) == USART1_RX_BUF[2]+4){
					
					USART1_RX_STA|=0x8000;
				}
			}
			else if((USART1_RX_STA&0X3FFF) == 3){
				USART1_RX_STA|=0x4000;
			}
			
			USART1_RX_BUF[USART1_RX_STA&0X3FFF]=Res;
			USART1_RX_STA++;
			if((USART1_RX_STA&0X3FFF)>(40-1))	USART1_RX_STA=0;
		}
	}
}
...
void USART2_IRQHandler(void){
	...
	if(USART_GetITStatus(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400)), ((uint16_t)0x0525)) != RESET)  
		{
		Res = USART_ReceiveData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400)));	
