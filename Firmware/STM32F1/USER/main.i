...
int main(void)
{
	...
	init_bsptimer();
	autotimer *mytimer = obtainTimer(0);
	TIM2_PWM_Init(719,9);	 
	while(1){
		printf("开始发送\r\n");
		im1281b_ask();
		printf("发送结束\r\n");
		while(im1281b_analysis_data() == 0){
		}
		printf("收到回应:TEMP:%f\r\n",Temp_data*0.01);
		USART1_RX_STA = 0;
		delay_ms(100);
	...
	while (1)
	{
		if (USART2_RX_STA & 0x8000)
		{
			...
			printf("comp:%d\r\n",comp);
			TIM_SetCompare1(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0000)),comp);
			for (t = 0; t < len; t++)
			{
				USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400)), USART2_RX_BUF[t]); 
				while (USART_GetFlagStatus(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4400)), ((uint16_t)0x0040)) != SET)
					; 
			}
			printf("\r\n\r\n"); 
